...
const addToActionQueue = (queue, actionId, request, resolveHandler) => {
  ...
  if (isManifestV3) {
    return new Promise((resolve) => {
      ...
      if (background.connectionStream.readable) {
        try {
          promisifiedBackground[method](...args).then((result) => {
            resolve(result);
...
const clearPendingActions = async () => {
  for (let i = 0; i < promisifiedActionRetryQueue.length; i++) {
    ...
    submitRequestToBackground(
      ...
      item.request.args,
      item.actionId,
    ).then((result) => item.resolveHandler(result));
  }
